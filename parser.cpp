...
bool Parser::parseInputFile(const std::string& filename, Placement& placement) {
    ...
    // 逐行读取文件内容
    /*
     * 使用get-line函数逐行读取文件内容，将每行内容赋值给字符串变量line，
     * 函数使用字符串流将line中的第一个标记提取出来，并将其赋值给字符串变量token
     */
    while (getline(fin, line)) {
        ...
        if (token == "DieSize") {
            ...
            ss >> placement.numInstances; // 读取 NumInstances(单元数)对应的值并存入 numInstances 中
        } else if (token == "Inst") {
            Unit unit;
            ss >> unit.name;
            placement.units.push_back(unit); // 将单元名添加到 units 容器中
        } else if (token == "NumNets") {
            ss >> placement.numNets; // 读取 NumNets对应的值(线网数)并存入 numNets 中
        } else if (token == "Net") {
            Net net;
            ss >> net.name; // 读取 Net 对应的值(线网名)并存入 net 的 name 中
            int numPins;
            ...
            for (int i = 0; i < numPins; i++) { // 循环读取每个线网连接的单元
                ...
                ss >> connectedUnit;
                net.connectedUnits.push_back(connectedUnit); // 将单个线网连接的单元添加到 net 的 connectedUnits 中
            }
